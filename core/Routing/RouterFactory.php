...
class RouterFactory{ 
    ...
    function __construct(
        ...
        if($arrAuthResponse['status'] === Response::SERVER_DB_ERR_STATUS){
    ...
    public function create(): ContextRouter{
        //vì $this->arrAuthResponse['data']['roles'] có format dạng array [strRoleCode => strDisplayName...] nên phải đùng
        //array_keys để lọc các roleCode ra
        $arrUserRole = array_keys($this->arrAuthResponse['data']['roles']);
        $arrEnabledModule = $this->arrAuthResponse['data']['registered_modules'];
