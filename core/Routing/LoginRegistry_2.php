...
final class LoginRegistry
{
    ...
    public const FIELD_DEFAULT_REDIRECT        = 'default_redirect';
    ...
    public function __construct(
        ...
        $arrLoginConfig = $this->loadConfig();

        $arrLoginConfig = $this->normalizeControllerNames(
            $arrLoginConfig
        );
        ...
        foreach (
            $arrLoginConfig as
            $strControllerName => $mixLoginConfig
        ) {
            ...
            $arrLoginRegistry[$strControllerName] =
    ...
    protected function loadConfig(): array
    {
        ...
        $mixLoginConfig = require $strConfigFilePath;

        if (!is_array($mixLoginConfig)) {
            throw new UnexpectedValueException(
                "File cấu hình login '{$strConfigFilePath}' "
                . 'phải trả về array; nhận được '
                . get_debug_type($mixLoginConfig)
                . '.'
            );
        }

        return $mixLoginConfig;
    }

    /*---------------------------------------------------------------------------------------------------------------*/
    protected function normalizeControllerNames(
        array $arrLoginConfig
    ): array {
        $arrNormalizedLoginConfig = [];
        ...
        foreach (
            ...
            $arrNormalizedLoginConfig[$strControllerName] =
                $mixControllerConfig;
        }

        return $arrNormalizedLoginConfig;
    }
    ...
    protected function buildRegistryEntry(
        ...
        $mixLoginConfig = $this->normalizeRememberCookie(
            $mixLoginConfig,
            $strContext
        );
    ...
    protected function validateDefaultRedirect(
        ...
        $strDefaultRedirect =
            $arrRegistryEntry[self::FIELD_DEFAULT_REDIRECT];
